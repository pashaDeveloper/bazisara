import React, { useState } from "react";

function Banner() {
  const confrimedBanner = localStorage.getItem("banner_confrimed");
  const [bannerOpen, setBannerOpen] = useState(confrimedBanner === "false" ? false : true);

  return (
    <>
      {bannerOpen && (
        <div className="fixed bottom-0 right-0 z-50 w-full md:bottom-8 md:right-12 md:w-auto">
          <div className="flex justify-between border border-zinc-700 bg-black p-3 text-sm text-zinc-200 shadow-lg md:rounded">
            <div className="inline-flex text-zinc-400">
              <p>
                برای دریافت تازه‌ترین خبرها و پیشنهادهای ویژه، اعلان‌های سایت را دنبال کنید.
              </p>
            </div>
            <button
              className="ml-3 border-r border-zinc-700 pr-2 text-zinc-500 transition hover:text-white"
              onClick={() => {
                localStorage.setItem("banner_confrimed", false);
                setBannerOpen(false);
              }}
            >
              <span className="sr-only">Close</span>
              <svg className="h-4 w-4 shrink-0 fill-current" viewBox="0 0 16 16">
                <path d="M12.72 3.293a1 1 0 00-1.415 0L8.012 6.586 4.72 3.293a1 1 0 00-1.414 1.414L6.598 8l-3.293 3.293a1 1 0 101.414 1.414l3.293-3.293 3.293 3.293a1 1 0 001.414-1.414L9.426 8l3.293-3.293a1 1 0 000-1.414z" />
              </svg>
            </button>
          </div>
        </div>
      )}
    </>
  );
}

export default Banner;


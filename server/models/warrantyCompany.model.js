...
warrantyCompanySchema.pre("save", async function (next) {
  try {
    ...
    if (!this.companyId) {
      const counter = await Counter.findOneAndUpdate({ name: "warrantyCompanyId" }, { $inc: { seq: 1 } }, { new: true, upsert: true });
...
module.exports = mongoose.model("WarrantyCompany", warrantyCompanySchema);
